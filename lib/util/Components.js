...
var componentDetectorUtil = require('./component-detector');
...
function componentRule(rule, context) {
  ...
  var componentDetector = componentDetectorUtil.getFromContext(context);
  ...
  var utils = {
    ...
    isES6Component: function(node) {
      ...
      var superClassText = sourceCode.getText(node.superClass);
      return new RegExp('^(' + pragma + '\\.)?(Pure)?Component$').test(superClassText)
        || (componentDetector && componentDetector.test(superClassText));
    },
